...
router.get("/owner-folders/files/:folder_id", (req, res) => {
  ...
  db.query(query, [folder_id], (err, results) => {
    ...
    // Check if we need to build URLs for file paths
    results.forEach(file => {
      // Keep the file_data as is, whether it's base64 or a path
      // The client will handle both formats
      if (file.file_data && !file.file_data.startsWith('data:')) {
        console.log(`File ${file.file_id} has path: ${file.file_data}`);
      }
    });
...
router.get("/portfolio-image-file", (req, res) => {
  ...
  console.log("Serving image file, path:", filePath);
