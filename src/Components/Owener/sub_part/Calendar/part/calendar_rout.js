...
router.post("/add-event-with-success", (req, res) => {
  ...
  // Directly insert the event, assuming user_email is valid and exists in the owner table
  const query =
    "INSERT INTO events (title, start, end, description, backgroundColor, user_email, team_members) VALUES (?, ?, ?, ?, ?, ?, ?)";

  db.query(
    query,
    [title, start, end, description, backgroundColor, user_email, JSON.stringify(team_members)],
    (err, result) => {
      if (err) {
        console.error("Error creating event:", err);
        return res.status(500).json({ error: "Failed to create event" });
