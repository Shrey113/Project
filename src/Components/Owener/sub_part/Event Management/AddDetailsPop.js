...
const AddDetailsPop = ({ setShowEventModal, newEvent, setNewEvent, set_receiver_package_data, set_receiver_equipment_data, set_receiver_service_data, profile_data }) => {
  ...
  useEffect(() => {
    if (newEvent.id) {
      socket.on(`event_member_response_${newEvent.id}`, (data) => {
        console.log("Team member response:", data);
        
        setAssignedMembers(prevMembers => 
          prevMembers.map(member => 
            member.member_id === data.member_id 
              ? { ...member, confirmation_status: data.status === "Accepted" ? "Accepted" : data.status } 
              : member
          )
        );
        
        if (data.event_status) {
          // Convert "Accepted" status to "Accepted" for UI display
          const displayStatus = data.event_status === "Accepted" ? "Accepted" : data.event_status;
          setNewEvent(prev => ({ ...prev, event_status: displayStatus }));
        }
      });
      
      socket.on(`event_status_changed_${newEvent.id}`, (data) => {
        console.log("Event status changed:", data);
        // Convert "Accepted" status to "Accepted" for UI display
        const displayStatus = data.status === "Accepted" ? "Accepted" : data.status;
        setNewEvent(prev => ({ ...prev, event_status: displayStatus }));
      });
      
      fetchTeamConfirmationStatus(newEvent.id);
    }
    
    return () => {
      if (newEvent.id) {
        socket.off(`event_member_response_${newEvent.id}`);
        socket.off(`event_status_changed_${newEvent.id}`);
      }
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [newEvent.id]);

  const fetchTeamConfirmationStatus = async (eventId) => {
    try {
      const response = await fetch(`${Server_url}/team_members/check-event-team-status`, {
        method: "POST",
      ...
      console.log("Team confirmation status:", data);
      
      if (data.members && data.members.length > 0) {
        // Convert any "Accepted" status to "Accepted" for UI display
        const membersWithUpdatedStatus = data.members.map(member => ({
          ...member,
          confirmation_status: member.confirmation_status === "Accepted" ? "Accepted" : member.confirmation_status
        }));
        setAssignedMembers(membersWithUpdatedStatus);
      }
      
      setTeamResponseStatus(data.status_counts);
      ...
      console.error("Error fetching team confirmation status:", error);
    }
  };
