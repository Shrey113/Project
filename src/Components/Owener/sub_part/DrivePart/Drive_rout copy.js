...
router.post('/get_all_file_size', (req, res) => {
    const { user_email } = req.body;
    const query = `SELECT SUM(file_size) as total_size FROM drive_files WHERE user_email = ?`;
    db.query(query, [user_email], (err, results) => {
        if (err) {
            return res.status(500).send('Error calculating storage: ' + err.message);
        }
        res.status(200).json(results[0].total_size || 0);
    });
});
