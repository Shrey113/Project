...
const PopUp = ({ action, member, onClose, onSave }) => {
    ...
    const validateForm = () => {
        ...
        if (!formData.member_event_assignment) errors.member_event_assignment = 'Event Assignment is required';
    ...
    const handleSubmit = async () => {
        ...
        if (action === 'Add') {
            ...
            try {
                const response = await fetch(`${Server_url}/team_members/update_member`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                    },
            ...
            <div className="popup-content">
                ...
                {action !== 'View' ? (
                    <form>
                        <label>
                            Name:
                            <input type="text" name="member_name" value={formData.member_name} onChange={handleChange} />
                            {formErrors.member_name && <div className="error">{formErrors.member_name}</div>}
                        </label>
                        <label>
                            Role:
                            <input type="text" name="member_role" value={formData.member_role} onChange={handleChange} />
                            {formErrors.member_role && <div className="error">{formErrors.member_role}</div>}

                        </label>
                        <label>
                            Event Assignment:
                            <input type="text" name="member_event_assignment" value={formData.member_event_assignment} onChange={handleChange} />
                            {formErrors.member_event_assignment && <div className="error">{formErrors.member_event_assignment}</div>}
                        </label>
                        <label>
                            Status:
                            <select name="member_status" value={formData.member_status} onChange={handleChange}>
                        ...
                        <div className="profile-pic-selection">
                            <img
                                ...
                                className={formData.member_profile_img === profile_pic_user1 ? 'selected' : ''}
                            />
                            <img
                                src={profile_pic_user2}
                                alt="User 2"
                                onClick={() => handleProfilePicChange(profile_pic_user2)}
                                className={formData.member_profile_img === profile_pic_user2 ? 'selected' : ''}
                            />
                            <img
                                src={profile_pic_user3}
                                alt="User 3"
                                onClick={() => handleProfilePicChange(profile_pic_user3)}
                                className={formData.member_profile_img === profile_pic_user3 ? 'selected' : ''}
                            />
                            <img
                                src={profile_pic_user4}
                                alt="User 4"
                                onClick={() => handleProfilePicChange(profile_pic_user4)}
                                className={formData.member_profile_img === profile_pic_user4 ? 'selected' : ''}
                            />
                        </div>
                    </form>
                ) : (
                    ...
                    <table className="member-table">
                      <tbody>
                        ...
                        <tr>
                          <td className="label"><strong>Role:</strong></td>
                          <td>{formData.member_role}</td>
                        </tr>
                        <tr>
                          <td className="label"><strong>Event Assignment:</strong></td>
...
const TeamOverview = () => {
    ...
    const [popupState, setPopupState] = useState({ show: false, action: '', member: null });
